...
17. `Partial`  
    ...
    Same-node owned facilities and stationed guards now feed temporary battle support cards into the player's deck, owned-building pass-by events can also queue next-battle prep cards, and player victories now offer post-battle card rewards, further linking map control to combat in `index.html`.
