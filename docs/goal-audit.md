...
17. `Partial`  
    ...
    Reserve deployment is also no longer a plain discount: different buildings now convert incoming reserves differently, for example fortresses and guard posts turn shield troops into thicker wall value, income sites turn scout reserves into immediate payback and supply prep, command posts can spin veteran reserves into rally tempo, and training-heavy buildings can squeeze extra attack and experience out of the same body. That gives the player a real reason to care not just about who got captured, but where those captured troops get placed next.  
    Owned building pass-by events now also start feeding that reserve layer directly: barracks can mint fresh reserve troops, while training camps, healing sites, and command posts can retune and drill reserves toward different battlefield roles over time. That means reserve management is no longer only a siege aftermath mechanic; it is becoming a standing part of peacetime route planning too.  
